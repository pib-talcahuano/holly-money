...
| **Encontrado en Task 5:** `lib/supabase/server.ts` cachea `role_permissions` por rol con `unstable_cache` (24h, tag `role-permissions`). Sembrar `MANAGE_BUDGETS` por migración SQL no invalida ese caché — un server ya corriendo (dev local u otro entorno persistente) sigue sin ver el permiso nuevo hasta que alguien togglee cualquier permiso en Settings → Permisos (dispara `revalidateRolePermissions()`) o pasen las 24h | Medio — en local hizo parecer "no funciona" cuando sí funcionaba; en un deploy real podría dejar el tab de Presupuesto invisible para ADMIN/BURSAR hasta la primera interacción en Settings → Permisos | Documentado acá y en `tasks/todo.md`. Mitigación para el PR: mencionarlo explícitamente en la descripción para que quien revise sepa que debe tocar Settings → Permisos (o esperar el revalidate) después de aplicar la migración en cualquier entorno de prueba compartido. |
